...
const RecruiterDashboard = () => {
  ...
  const fetchCandidates = useCallback(async (portalId, silent = false) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/recruiter/candidates/${portalId}`
      );
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Invalid recruiter session:", err);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchCandidates(recruiter.portalId, true);
    }, 15000);
  ...
  const handleRegister = async (e) => {
    ...
    if (!name || !email || !password || !jobTitle || !company) {
      return setError("Fill all fields");
    }
  ...
  const updateStatus = async (id, status) => {
    try {
      await fetch(`${API_URL}/recruiter/candidate/${id}/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const filtered = [...candidates]
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "bias") {
        return b.biasFreeScore - a.biasFreeScore;
      }
        ...
        <div style={s.authCard}>
          ...
          <p style={s.authSub}>
            {view === "login"
              ...
              : "Set up your FairHire recruiter account"}
          </p>
        ...
        <div style={s.sideTop}>
          ...
          <div
            style={{
              ...s.avatar,
              background:
                "linear-gradient(135deg,#b8863a,#e8c073)",
            }}
          >
          ...
          <h3 style={s.sidebarName}>
            {recruiter?.name}
          </h3>

          <p style={s.sidebarEmail}>
            {recruiter?.email || ""}
          </p>
        </div>
      ...
      <div style={s.mainPanel}>
        ...
        <div style={s.dashHeader}>
          <div>
            ...
            <p style={s.dashSub}>
              Manage and shortlist candidates from
              your portal
            </p>
          </div>
          ...
          <button
            ...
            onClick={() =>
              fetchCandidates(recruiter.portalId)
            }
            disabled={refreshing}
          >
            {refreshing ? "⏳" : "🔄"} Refresh
                      ...
                      <div style={s.barBg}>
                        <div
                          style={{
                            ...
                            width: `${c.atsScore}%`,
                            background:
                              "linear-gradient(90deg,#b8863a,#e8c073)",
                          }}
                        />
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div style={s.barBg}>
                        <div
                          style={{
                            ...s.barFill,
                            width: `${c.biasFreeScore}%`,
                            background:
                              "linear-gradient(90deg,#2f8f6f,#4fceae)",
                          }}
                        />
                      </div>
                    </div>
...
const s = {
  ...
  orb1: {
    ...
    height: 500,
    background:
      "radial-gradient(circle,#b8863a,transparent)",
    bottom: -150,
    right: -150,
  },
  ...
  orb2: {
    ...
    height: 400,
    background:
      "radial-gradient(circle,#2f8f6f,transparent)",
    top: -100,
    left: -100,
  },
  ...
  sideLogoTxt: {
    ...
    fontSize: "1.1rem",
    background:
      "linear-gradient(135deg,#e8c073,#4fceae)",
